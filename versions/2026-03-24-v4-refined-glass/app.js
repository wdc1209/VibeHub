const STORAGE_KEY = 'openclaw-vibe-hub-v3-cloud-living-cn-v2';
...
function normalizeState() {
  state.ui = state.ui || { floatingPosition: null };
}
...
function bindEvents() {
  ...
  els.detailCloseBtn.addEventListener('click', clearDetailSelection);
  els.stackDrawer.addEventListener('click', handleDrawerBackgroundClick);
...
function centerCardIfNeeded(force = false) {
  ...
  const height = els.tokenCard.offsetHeight || 640;
...
async function handleSendCurrentCard() {
  ...
  state.currentCard.lastAction = `已送去 ${payload.target}`;
  hideFeedbackBadge();
  showToast(`已送去 ${payload.target}`);
  persistAndRender();
}
...
async function handleRewrite() {
  ...
  showFeedbackBadge(before !== result.body ? '云层已经被整理得更清晰' : '已经顺着当前气息重写过了', 'rewrite');
...
async function handleShorten() {
  ...
  showFeedbackBadge(before !== result.body ? '云层被收拢成更轻的一团' : '它已经足够轻了', 'shorten');
...
function handleCancel() {
  ...
  state.currentCard.lastAction = '这团云暂时不送了';
  hideFeedbackBadge();
  showToast('已取消');
  persistAndRender();
}
...
function handleDetailSendAnother() {
  ...
  state.currentCard = {
    ...
    body: card.body,
    rawInput: card.rawInput || card.body,
    lastAction: `准备转送到 ${nextTarget}`,
  };
  state.stackOpen = false;
  hideFeedbackBadge();
  showToast(`已切换到 ${nextTarget}，可确认后送出`);
  persistAndRender();
}
...
function handleDrawerBackgroundClick(event) {
  if (!state.selectedStackId) return;
  if (event.target.closest('.stack-card, .stack-detail, .close-detail-button')) return;
