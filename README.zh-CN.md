...
- 当前 `1.x` 产品线以 macOS 为主。
- 当前 GitHub 发布路线就是 macOS 用户的正式分发路径。
- macOS 版本继续保留苹果默认语音作为默认方案。
- `SenseVoice ONNX` 继续作为第二语音选项，且需要用户先下载本地模型。
- Windows 版本如果后续要做，会单独作为一个桌面安装包推出，而不是为了跨平台先改坏当前的 macOS 版本。
- 长期方向是尽量复用后端与路由逻辑，但前台桌面壳按平台分别维护。
